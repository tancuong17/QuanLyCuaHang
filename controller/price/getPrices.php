<?php
    ...
    $stament = $connect->prepare("SELECT price.id, product.id as idProduct, product.name, price.price, price.start_date, price.end_date FROM price INNER JOIN product ON product.id = price.id_product ORDER BY price.create_date DESC");
    $stament->execute();
    $prices = array();
