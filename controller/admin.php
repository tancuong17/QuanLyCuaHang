...
$prices = array();
$bills = array();
$stament = $connect->prepare("SELECT * FROM product LEFT JOIN price ON price.id_product = product.id ORDER BY product.id DESC");
$stament->execute();
...
$stamentPrice = $connect->prepare("SELECT product.id, product.name, price.price, price.start_date, price.end_date FROM price INNER JOIN product on price.id_product = product.id ORDER BY price.create_date DESC");
$stamentPrice->execute();
while ($row = $stamentPrice->fetch()) {
    $price = number_format($row['price'], 0, '.', ',');
    array_push($prices, array("productId" => $row['id'], "nameProduct" => $row["name"], "price" => $price . "đ", "startDate" => $row["start_date"], "endDate" => $row["end_date"]));
}
